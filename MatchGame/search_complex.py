...
from judge_equality import trans_single_list,trans_list_single
from complex_judge import judge
#长等式部分的搜索，judge采用正则表达式方法，调用complex_judge.py中的judge函数，这是本文件与search.py唯一不同的，
# 搜索算法部分完全相同
...
def Searchcom_2(list):
    ...
    list_search_1,list_search_1_judge_list = Search_com(list)
    #print("-----------------------------------------------------------------------------------------------------------")
    #自身移动两次火柴
    for i in single:
        for j in all_possibility[i]:
                ...
                for k in pos_1:
                    for t in pos_2:
                        if (k != t):
                            ...
                            single_copy[t] = j[3]
                            list_judge = trans_single_list(single_copy)
                            list_add,list_search_1_judge = Search_com(list_judge)
...
if __name__ == '__main__':
    list = [9,'-',5,'=',8]
    list_dis,list_jud=Search_com(list)
    print(list_dis)
    print(len(list_jud))
